...
//		String c = erg.toString();
	...
	// alle Datenbankeintr�ge ein, die in jedem Monom vorkommen.
	...
	// Berechnet einen lazyGcd diese RExpressions mit dem �bergebenen RExpMonomial, d.h. alle
	// gemeinsamen Anteile der Faktoren und alle Datenbankeintr�ge, die in jedem Monom vorkommen.
	// Ist das RExpMonomial 0, so wird in unver�ndert zur�ckgegeben.
	...
	// Diese Routine eliminiert die Wurzeln f�r den Test auf 0. Daf�r wird
	...
	// Variablen mit einem Wert belegt, und liefert das ausgewertete Polynom zur�ck.
 ...
 * @param signs int[]
	...
	sein und hat signs genau soviele Eintr�ge 0 oder 1, wie es verschiedene Wurzeln in der Darstellung
	gibt, so gibt diese Methode die durch signs angegebene Konjugierte zur�ck.
...
public int getDepth() 
{
	...
	// liefert den h�chsten vorkommenden Index (i.A. die Anzahl der Variablen).
...
public boolean isCertainlyOne() 
{
	...
	// L�sst sich sp�ter vielleicht noch ausbauen.
	public boolean isCertainlyZero()
	{
		return monom.size()==0;
	}
	public static boolean isDatabaseEnabled() {return datenbankAktiv;}
	// nicht equals �berschreiben, sonst sind 1+sqrt(2) und 1sqt(2) identisch!
	...
	// Gibt eine Liste zur�ck, mit welchem RExpression man den ggT
	// multiplizieren m�sste, um auf den jeweiligen factor zu kommen.
	...
	// Findet einen RExpression, der alle FastPolynomial- und alle RExpression-Datenbankeintr�ge
	// enth�lt, die in allen RExpressionen des Arrays v_in vorkommen. 
	...
	// Gibt eine Liste zur�ck, mit welchem RExpression man die jeweiligen
	...
	// in jedem monom auftauchen m�ssen. Aufgerufen wird diese Routine von RQuotientenExp,
	// um zu k�rzen, und zwar mit dem commonContent united mit dem commonContent eines anderen
	// RExpression. 
